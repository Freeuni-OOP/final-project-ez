...
class CompositionServiceTest {
    ...
    @BeforeEach
    void setUp() {
        compositionService = new CompositionService(compositions, users, tagService);
        lenient().when(tagService.resolve(any())).thenReturn(Set.of());
    }
    ...
    void create_savesACompositionOwnedByTheCurrentUser() {
        ...
        CompositionRequest request = new CompositionRequest("My Song", "C4 E4 G4", 120, List.of());
    ...
    @Test
    void create_attachesTheTagsResolvedByTagService() {
        com.algorythm.model.Tag lofi = new com.algorythm.model.Tag("lofi");
        when(users.findByUsername("alice")).thenReturn(Optional.of(alice));
        when(compositions.save(any(Composition.class))).thenAnswer(inv -> inv.getArgument(0));
        when(tagService.resolve(List.of("Lofi", "chill"))).thenReturn(Set.of(lofi));


        compositionService.create(
                "alice", new CompositionRequest("Song", "pattern", 120, List.of("Lofi", "chill")));


        ArgumentCaptor<Composition> captor = ArgumentCaptor.forClass(Composition.class);
        verify(compositions).save(captor.capture());
        assertThat(captor.getValue().getTags()).containsExactly(lofi);
    }
    ...
    void create_rejectsWhenTheCurrentUserIsUnknown() {
        ...
        CompositionRequest request = new CompositionRequest("Song", "pattern", 120, List.of());
    ...
    void update_updatesFieldsOnACompositionOwnedByTheCurrentUser() {
        ...
        when(users.findByUsername("alice")).thenReturn(Optional.of(alice));
        when(compositions.findByIdAndOwner(5L, alice)).thenReturn(Optional.of(owned));
        CompositionRequest request = new CompositionRequest("New title", "new pattern", 140, List.of());
    ...
    @Test
    void update_replacesTheTagsWithWhateverTagServiceResolves() {
        com.algorythm.model.Tag energetic = new com.algorythm.model.Tag("energetic");
        Composition owned = new Composition(alice, "Song", "pattern", 100);
        owned.setTags(Set.of(new com.algorythm.model.Tag("chill")));
        when(users.findByUsername("alice")).thenReturn(Optional.of(alice));
        when(compositions.findByIdAndOwner(5L, alice)).thenReturn(Optional.of(owned));
        when(tagService.resolve(List.of("energetic"))).thenReturn(Set.of(energetic));


        compositionService.update(
                "alice", 5L, new CompositionRequest("Song", "pattern", 100, List.of("energetic")));


        assertThat(owned.getTags()).containsExactly(energetic);
    }
